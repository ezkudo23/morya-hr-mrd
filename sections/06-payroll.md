...
> **Status:** ✅ Locked — อัปเดต 2 พ.ค. 2569
> **Scope:** Payroll engine + Payslip + รายการหักพิเศษ + โบนัส
...
วันที่ 1-14:         Prep Round 2 (OT + commission + เบี้ยขยัน + โบนัส)
...
ก่อน run_payroll ต้องเช็ค:
1. **Shift Assignment** — ทุกคนต้องมี employee_shifts → ถ้าไม่มี REJECT + แจ้งรายชื่อ
2. **Late Detection** — attendance_logs ที่ shift_id IS NULL → WARNING

ดู Section 5.7 — Pre-Run Validation
...
### OT Types (🔴 อัปเดต 2 พ.ค. 2569 — 3 type)

ดู Section 5.3 — OT Multiplier Table

| ot_type | rate_multiplier | กรณีใช้งาน |
|---------|-----------------|-----------|
| `normal` | 1.5x | วันทำงานปกติ |
| `holiday` | 3.0x | วันหยุด (closed) หรือ วันหยุดร้านที่สั่งเปิด (open_changed) |
| `substitute` | 1.5x | วันหยุดที่ร้านเปิดปกติ (open_substitute) — พนักงานได้ Token แทน |

### Calculation

```
ot_amount = (salary_base / 30 / 8) × rate_multiplier × hours
```
...
จาก `holiday_calendar` (ดู Section 5.3) — เก็บ snapshot ใน `ot_requests.ot_type`
...
## 6.7 Bonus (🔴 ใหม่ 2 พ.ค. 2569)

**ที่มาของ Decision:** โบนัสประจำปี 2 ครั้ง/ปี ต้องแสดงแยกใน Slip ไม่ปะปนกับ "รายได้อื่นๆ"

### Field

`payroll_details.bonus_amount` — DECIMAL(10,2) NOT NULL DEFAULT 0

### Use Cases

- **โบนัสประจำปี** เช่น ปีใหม่, สงกรานต์ — HR Admin กรอก
- **โบนัสพิเศษรายบุคคล** — กรณี Owner ตัดสินใจให้
- จ่ายได้ทั้ง Round 1 และ Round 2 (ปกติ Round 2)

### Display

- Slip: แสดงในส่วน "รายได้" แยกบรรทัด "โบนัส"
- YTD: แสดง "โบนัสสะสมประจำปี" แยก
- Report: แยกออกจาก gross_income ได้

---

## 6.8 LWP (Leave Without Pay)
...
## 6.10 รายการหักพิเศษ — payroll_deductions
...
```
ลาไม่รับค่าจ้าง LWP (lwp_deduction)
...
### YTD ที่แสดง (🔴 อัปเดต 2 พ.ค. 2569)

```
รายได้สะสม (gross_income YTD)
โบนัสสะสม (bonus YTD)                         ← ใหม่
ภาษีสะสม (wht YTD)
SSO ลูกจ้างสะสม
SSO นายจ้างสมทบสะสม
```
...
```json
{
  ...
  "income": { "base_salary", "worked_days", "absent_days", "prorated_salary", "ot_amount", "commission", "bonus_amount", "diligence_bonus", "diligence_forfeited", "other_income", "adjustment_r1", "adjustment_r2", "gross_income" },
  "deductions": { "lwp_days", "lwp_deduction", "sso_base", "sso_employee", "sso_employer", "wht_amount", "special_deductions": [] },
  "net_pay": 20000,
  "payslip_note": null,
  "ytd": { "gross_income", "bonus", "wht", "sso_employee", "sso_employer" },
...
## 6.13 system_config — Payroll & Company Info
...
### payroll_details (🔴 อัปเดต 2 พ.ค. 2569)
...
## 6.15 Payroll Rollback Procedure
...
| OT types | 3 ค่า (normal/holiday/substitute) | ✅ ใหม่ 2 พ.ค. |
| Commission | 40(1), Round 2 only | 🔒 D8 |
| Bonus | bonus_amount column แยก | ✅ ใหม่ 2 พ.ค. |
| Payslip | 1 ใบ/เดือน, วันที่ 16 | ✅ |
| Payslip layout | A4 แนวตั้ง | ✅ |
| รายการหักพิเศษ | payroll_deductions table | ✅ |
...
- [ ] Pre-Run Payroll Validation implement
...
*Last updated: 2 พ.ค. 2569 | bonus_amount column | OT 3 type (normal/holiday/substitute) | Pre-Run Payroll Validation*
